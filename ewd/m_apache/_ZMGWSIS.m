...
CHILDE ; Error
 d EVENT($ZS)
 i $ZS["READ" g HALT
 ; DLW - The end-of-file error condition string shows up in $zstatus
 ;       $zerror usually contains "Unprocessed $ZERROR, see $ZSTATUS"
 ;       $zerror is intended to be set by the application code, not GT.M
 ; i $ze["%GTM-E-IOEOF" g HALT
 i $zs["%GTM-E-IOEOF" g HALT
 G CHILD2
 ;
HALT ; Halt
 i 'conc d
...
web ; Web request
 ...
 K ^%MGWSI("WS",$J)
 x "n ("_refn_") d "_ref
 I $G(^%MGWSI("WS","STATUS"))'=-2 S status=$$wsexit()
 K ^%MGWSI("WS",$J)
 C 0
...
webx ; Exit
 q
webe ; error
 I $D(req3("HTTP_SEC_WEBSOCKET_KEY")) D EVENT^%ZMGWSIS("WebSocket Error: "_$ZE)
 S REQ=""
...
WS(CGI,DATA)
 ...
 new $ZTRAP set $ZTRAP="ZGOTO "_$ZLEVEL_":WSE^%ZMGWSIS"
 S STATUS=$$wsstart^%ZMGWSIS()
 S TIMEOUT=10
 S DATA=""
 F I=1:1:10 D  I DATA="exit" Q
 . N I
 . S STATUS=$$wsread^%ZMGWSIS(.DATA,TIMEOUT)
 . I STATUS=-2 S DATA="exit" Q
 . I STATUS=-1 S DATA="Timeout Event ("_TIMEOUT_"): $H="_$H_"; $ZV="_$ZV
 . I DATA="exit" Q
 . S STATUS=$$wswrite^%ZMGWSIS(DATA)
 . Q
 Q
WSE ; Error
 D EVENT^%ZMGWSIS("WebSocket Test Error: "_$ZE)
 Q
 ;
...
wsread(DATA,TIMEOUT) ; Websocket - read from client
 N STATUS,HEAD,TYPE
 new $ZTRAP set $ZTRAP="ZGOTO "_$ZLEVEL_":wsreade^%ZMGWSIS"
 S STATUS=1
...
wsreadx ; Exit
 I STATUS=-2 S ^%MGWSI("WS",$J,"STATUS")=STATUS
 Q STATUS
wsreade ; Error
 S ^%MGWSI("WS",$J,"STATUS")=-2
 Q -2
 ;
wswrite(DATA) ; Websocket - write to client
 N STATUS,HEAD,TYPE,SIZE
 new $ZTRAP set $ZTRAP="ZGOTO "_$ZLEVEL_":wswritee^%ZMGWSIS"
 S STATUS=1
 S TYPE=0
 S SIZE=$L(DATA)
...
SetUNPW(username,password)
 S ^%MGWSI(0,"username")=username
 S ^%MGWSI(0,"password")=password
 Q 1
 ;
GetUNPW(username,password)
 S username=$G(^%MGWSI(0,"username"))
 S password=$G(^%MGWSI(0,"password"))
 Q 1
 ;
